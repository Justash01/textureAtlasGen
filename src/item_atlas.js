...
var itemTexturePath = JSON.parse(settings).item_textures_path;
...
if (fs.existsSync(itemTexturePath)) {
  ...
  const result = getImages(itemTexturePath);
  ...
  fs.writeFile(`${itemTexturePath.split("textures")[0]}textures/item_texture.json`, JSON.stringify(data, null, jsonInd), function (err) {
    if (err) throw err;
  });
};